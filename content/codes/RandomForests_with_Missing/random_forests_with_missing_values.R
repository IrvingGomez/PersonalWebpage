...
# A regression tree
miss_regTree <- function(X, y, nodesize = 5, tn = Inf, X_space = NULL){
  
  Xdata <- X   # The original data
  ...
  if(length(h_miss)>0){
    X <- X[,-h_miss]
    warning("There were completely missing Variables, they were removed.")
  ...
  if(length(i_incomp)>0){
    if(length(i_miss)>0){
      warning("There were completely missing Observations, they were kept.")
    }else{
      warning("There were missing Observations.")
    }
  }
  ...
  if(!is.null(X_space)){
    ...
    if(sum(in_X_space)!=length(i_notcomp_miss)){
      ...
      warning("There were some points out of the X-space, they were removed.")
    }
  }else{
    minima <- c()
    maxima <- c()
  ...
  ##############################################
  ##                                          ##
  ## Here starts the construction of the tree ##
  ##                                          ##
  ##############################################
  
  # variables that will say the structure of the tree
  ...
  #X_whole  <- list(X) # the story of the partition
  probs    <- c(1)    # probability of being on left/right for missing values
  nodeID   <- c(1)
  contnode <- 1
  
  # partition of the observations and response generated by the tree partition 
  P  <- list(X_space)
  XP <- X %>% list()
  yP <- y %>% list()
  ...
  out <- list(Pfinal, X_space, Xfinal, yfinal,
              y_pred, Ssplits, Xdata, y)
  names(out) <- c('Partition', 'X_space', 'X.Partition',
                  'y.Partition', 'y.Predict',
                  'Structure', 'X', 'y')
  return(out)
}

# A random regression tree
miss_regTree_RF <- function(X, y, mtry = max(floor(ncol(X)/3),1),
  ...
  while(!length(P)==0){
    ...
    if(nA <= nodesize || length(unique(yA)) == 1 || tn_curr >= tn ||
      ...
      ##########################
      ##                      ##
      ## Select mtry features ##
      ##                      ##
      ##########################
      
      # features are candidates only if they have at least two points not missing
      cand <- c()
...
miss_regRF <- function(X, y, ntree = 100,
  ...
  trees <- foreach(j = 1:ntree,
                   ...
                   .export = ls(envir=globalenv())) %dopar% {
                     sel <- which(B[,j] == 0)
                     miss_regTree_RF(X[sel,], y[sel], mtry,
                                        nodesize, tn, X_space)}
  ...
  out <- list(trees, B, X_space, oob.err, X, y, Xdata, ydata, param)
  names(out) <- c('trees', 'OOBmatrix', 'X_space', 'OOB.err',
                  'X.keep', 'y.keep', 'X', 'y', 'RF.param')
  return(out)
}
...
miss_oob.err <- function(forest){
  ...
  y.pred <- foreach (k = 1:length(trees),
                     ...
                     .export = ls(envir = globalenv())) %dopar% {
                       ...
                       for(i in oob){
                         aux[i] <- miss_tree.pred(X[i,], trees[[k]])
                       }
...
miss_tree.conect <- function(x = NULL, y = NULL, tree){
  
  # Get the observations in the tree
  X    <- tree$X    # Points given to the tree
  Xp   <- tree$X.Partition
  
  X_tree <- Xp[[1]] # Points assigned in the tree
  if(length(Xp)>1){
    for(i in 2:length(Xp)){
      X_tree <- rbind(X_tree, Xp[[i]])
    }
  }
  
  rX_tree <- rownames(X_tree)
  rX      <- rownames(X)
  
  obs <- rX[rX %in% rX_tree]
  n   <- length(obs)
  ...
  if(is.null(x)){
    ...
    conect <- rep(0, n)
    y_pred_x <- miss_tree.pred(x, tree)
    y_pred_tree <- round(tree$y.Predict,3)
    
    leaf <- which(y_pred_tree == y_pred_x)
    Part <- tree$X.Partition[[leaf]]
    
    rX_node <- rownames(Part)
